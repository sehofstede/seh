	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('well'); ?> <?php sehof_attribute_lang( get_the_ID() ); ?>>
		...
		<header class="entry-header">
			...
			<h2 class="entry-title">
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'sehof' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
			</h1>
			<?php endif; // is_single() ?>
			<?php if ( comments_open() ) : ?>
				<div class="comments-link">
					<?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a reply', 'sehof' ) . '</span>', __( '1 Reply', 'sehof' ), __( '% Replies', 'sehof' ) ); ?>
				</div><!-- .comments-link -->
		...
		<div class="entry-content">
			<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'sehof' ) ); ?>

			<?php wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'sehof' ), 'after' => '</div>' ) ); ?>
		</div><!-- .entry-content -->
		<?php endif; ?>
